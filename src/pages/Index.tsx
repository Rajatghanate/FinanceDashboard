        ...
        <main className="p-3 sm:p-4 md:p-6 lg:p-8 max-w-[2000px] mx-auto">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-4">
            ...
            <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* My Cards - Top Left */}
              <div id="my-cards" className="h-full">
                <Card className="p-4 sm:p-5 md:p-6 bg-card h-full flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-white border border-gray-200 mt-auto">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <svg className="w-12 h-12 transform -rotate-90">
                          <circle cx="24" cy="24" r="20" fill="none" stroke="#e5e7eb" strokeWidth="4" />
                          <circle cx="24" cy="24" r="20" fill="none" stroke="hsl(var(--primary))" strokeWidth="4" strokeDasharray="126" strokeDashoffset="31.5" strokeLinecap="round" />
                        </svg>
                      </div>
                      <div>
              ...
              {/* Spending Summary - Top Right */}
              <div id="spending" className="h-full">
                <Card className="p-4 sm:p-5 md:p-6 h-full flex flex-col justify-between">
                  ...
                  <div className="flex items-end justify-center mb-6">
                    <div className="relative w-64 h-32">
                      <svg className="w-full h-full" viewBox="0 0 200 100">
                        <path d="M 20 95 A 80 80 0 0 1 98.6 15" fill="none" stroke="#2563EB" strokeWidth="20" strokeLinecap="butt" />
                        <path d="M 101.4 15 A 80 80 0 0 1 167.8 52.6" fill="none" stroke="#38BDF8" strokeWidth="20" strokeLinecap="butt" />
                        <path d="M 169.3 55 A 80 80 0 0 1 180 95" fill="none" stroke="#E5E7EB" strokeWidth="20" strokeLinecap="butt" />
                      </svg>
                  ...
                  <div className="mt-auto p-3 rounded-xl bg-primary/5 text-center">
                    <div className="text-sm text-muted-foreground">Your weekly spending limit is $2000.</div>
                  </div>
                </Card>
              </div>

              {/* Recent Transactions - Bottom Left - UPDATED SPACING */}
              <div id="transactions" className="h-full">
                <Card className="p-6 h-full flex flex-col">
                  ...
                  <div className="flex gap-3 mb-6 overflow-x-auto pb-2 scrollbar-hide">
                    <button className="px-4 py-2 text-sm font-medium rounded-lg bg-card-foreground text-background whitespace-nowrap">
                      Incoming
                    </button>
                    <button className="px-4 py-2 text-sm font-medium rounded-lg text-muted-foreground hover:bg-muted whitespace-nowrap transition-colors">
                      Outgoing
                    </button>
                    <button className="px-4 py-2 text-sm font-medium rounded-lg text-muted-foreground hover:bg-muted whitespace-nowrap transition-colors">
                      Pending
                  ...
                  <div className="space-y-6 flex-1">
                    {[
                      ...
                      <div key={index} className="flex items-center justify-between py-1 hover:bg-muted/30 transition-colors rounded-lg px-2 -mx-2 cursor-pointer group">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${transaction.bgColor}`}>
                            <span className="text-xl">{transaction.icon}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-semibold text-card-foreground truncate">{transaction.name}</div>
                            <div className="text-xs text-muted-foreground truncate hidden sm:block">{transaction.description}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0">
                          <div className="text-right">
                            <div className="text-base font-semibold text-card-foreground whitespace-nowrap">
                              {transaction.amount}
                            </div>
                            <div className="text-xs text-muted-foreground whitespace-nowrap">{transaction.date}</div>
                          </div>
                          <ArrowUpRight className="h-4 w-4 text-muted-foreground group-hover:text-card-foreground transition-colors flex-shrink-0" />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Subscriptions - Bottom Right */}
              <div id="subscriptions-card" className="h-full">
                <SubscriptionsCard className="h-full" />
              </div>
            </div>
